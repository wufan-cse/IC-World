...
def main(args):
    ...
    latents_txt_path = args.prompt_file
    ...
    for _, data in tqdm(enumerate(train_dataloader), disable=local_rank != 0):
        with torch.inference_mode():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                prompt = [c.split(".png:")[-1] for c in data["caption"]]
                image_names = [c.split(".png:")[0] for c in data["caption"]]
                ...
                for idx, video_name in enumerate(data["filename"]):
                    ...
                    item["caption"] = prompt[idx]
                    image_filename = image_names[idx] + ".png"
                    
                    # Add image path to JSON data
                    # TODO: clean this code
                    if args.is_static_scene:
                        item["image_path"] = "./data/IC-World-dataset/static_scene_dynamic_camera_first_frames/" + image_filename
                    else:
                        item["image_path"] = "./data/IC-World-dataset/dynamic_scene_static_camera_first_frames/" + image_filename
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_type", type=str, default="wan", 
    ...
    parser.add_argument(
        ...
        type=int,
        default=28,
        help="Number of inference steps for Flux image generation"
    )
    parser.add_argument(
        "--guidance_scale",
        type=float,
        default=3.5,
        help="Guidance scale for Flux image generation"
    )
    parser.add_argument(
        "--is_static_scene",
         action="store_true"

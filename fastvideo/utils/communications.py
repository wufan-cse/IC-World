...
def prepare_sequence_parallel_data_ic(
    encoder_hidden_states, encoder_attention_mask, caption, image_path
):
    if nccl_info.sp_size == 1:
        return (
            encoder_hidden_states,
    ...
    return encoder_hidden_states, encoder_attention_mask, caption, image_path


def sp_parallel_dataloader_wrapper_ic(
    dataloader, device, train_batch_size, sp_size, train_sp_batch_size
):
    while True:
        for data_item in dataloader:
            cond, cond_mask, caption, image_path = data_item
            #latents = latents.to(device)
            cond = cond.to(device)
            #attn_mask = attn_mask.to(device)
            cond_mask = cond_mask.to(device)
            ...
            frame = 19
            if frame == 1:
                yield cond, cond_mask, caption, image_path
            else:
                cond, cond_mask, caption, image_path = prepare_sequence_parallel_data_ic(
                    cond, cond_mask, caption, image_path
                )
                assert (
                    train_batch_size * sp_size >= train_sp_batch_size
                ), "train_batch_size * sp_size should be greater than train_sp_batch_size"

...
def save_lora_checkpoint(transformer, optimizer, rank, output_dir, step,
                         pipeline, epoch):
    # Check if transformer is wrapped with FSDP
    is_fsdp = hasattr(transformer, '_fsdp_wrapped_module')
    ...
    if is_fsdp:
        ...
        # For non-FSDP models, get state dict directly
        full_state_dict = transformer.state_dict()
    ...
    if rank <= 0:
        ...
        if pipeline is not None:
            ...
            # Fallback: save LoRA weights directly
            lora_weights_path = os.path.join(save_dir, "pytorch_lora_weights.safetensors")
            from safetensors.torch import save_file
            save_file(transformer_lora_layers, lora_weights_path)

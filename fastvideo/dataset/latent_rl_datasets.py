...
class LatentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        caption = self.data_anno[idx].get("caption", "")
        image_path = self.data_anno[idx].get("image_path", None)
        
        if image_path is not None:
            return prompt_embed, prompt_attention_mask, caption, image_path
            # return prompt_embed, prompt_attention_mask, caption, os.path.join(self.datase_dir_path, image_path)
        else:
            return prompt_embed, prompt_attention_mask, caption
...
def latent_image_collate_function(batch):
    # return latent, prompt, latent_attn_mask, text_attn_mask
    # latent_attn_mask: # b t h w
    # text_attn_mask: b 1 l
    # needs to check if the latent/prompt' size and apply padding & attn mask
    prompt_embeds, prompt_attention_masks, caption, image_path = zip(*batch)
    # attn mask
    prompt_embeds = torch.stack(prompt_embeds, dim=0)
    prompt_attention_masks = torch.stack(prompt_attention_masks, dim=0)
    #latents = torch.stack(latents, dim=0)
    return prompt_embeds, prompt_attention_masks, caption, image_path
